    ...
    try {
      ...
      for (const collection of collectionsForScripts) {
        ...
        const scriptSource = generateMigrationScriptForCollection(
          collection,
          Array.from(deps),
          analysisForHtml.sqlSchema.tables.find((t) => t.name === collection.name),
        );
...
function generateMigrationScriptForCollection(
  ...
  // Build related data loading code if dependencies exist
...
${dependencies
  ...
  // Build related lookup code for building docs
  const relatedLookupCode = dependencies
    .map((dep) => {
      // Try to find FK field that references this dependency
      const fkField =
...
// This script migrates data from Postgres table "${tableName}" to MongoDB collection "${collection.name}".
// Dependencies: ${dependencies.length > 0 ? dependencies.join(", ") : "none"}
// Run this script: node ${collection.name}.migrate.js
...
async function migrate() {
  ...
  console.log(\`Migrating ${collection.name} from Postgres to MongoDB...\`);
...
${relatedLookupCode}
      ...
      // Determine ID field for upsert
      const docId = doc["${idField}"] ?? doc["${pascalCase(idField)}"] ?? doc._id;
      if (docId == null) {
        console.warn(\`Row missing ID field "${idField}", skipping:\`, row);
        continue;
      }
    ...
    console.log(\`✓ Migrated \${migrated} documents into "\${collectionPrefix}${collection.name}" collection.\`);
  } finally {
    await pg.end().catch(() => {});
...
function renderNoSqlFieldsMapping(
  ...
  for (const field of fields) {
    if (field.type === "object" && field.fields && field.fields.length > 0) {
      ...
      const nestedObject = renderNoSqlFieldsMapping(field.fields, indent + 2, nestedPath);
...
function generateMigrationRunnerScript(collections: NoSqlCollection[]): string {
  type CollectionInfo = {
    name: string;
    moduleVar: string;
    funcName: string;
  ...
  function visit(name: string) {
    ...
    if (tempMark.has(name)) {
      // cycle; just return to avoid infinite loop
      return;
    }
    tempMark.add(name);
  ...
  return `// Auto-generated by sql2nosql.\n` +
