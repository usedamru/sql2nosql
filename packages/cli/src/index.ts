...
${dependencies
  ...
  // Index creation: PK (unique), then FK columns
...
`
  ...
  // Composite PK: build filter object and skip if any PK missing
...
${singlePkFilterBuild}
    ...
    console.log(\`✓ Migrated \${migrated} documents into "\${collectionPrefix}${collection.name}"\${dryRun ? " (dry-run)" : ""}. Total rows: \${totalRows}\${errors > 0 ? \`, errors: \${errors}\` : ""}\`);
    // --- END GENERATED ---
  } finally {
    await pg.end().catch(() => {});
